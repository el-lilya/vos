...
# CUDA_VISIBLE_DEVICES=$DEVICE python detection/apply_net.py --dataset-dir data --test-dataset coco_ext_fruits_val  --config-file $CONFIG --inference-config Inference/standard_nms.yaml  --random-seed $RANDOMSEED --image-corruption-level 0 --visualize $VISUALIZE --savefigdir $SAVEFIGDIR
